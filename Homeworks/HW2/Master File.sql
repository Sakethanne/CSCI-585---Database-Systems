...
select 
sponsors.name as "Sponsor Name", sponsors.contact as "Sponsor Phone Number", sum(sponsors.amount) as "Total Sponsored Amount" 
from sponsors 
group by sponsors.name, sponsors.contact 
order by sum(sponsors.amount) desc 
limit 1;

-- query 2 v1

select 
videos.title as "Video Title", channels.name as "Channel Name", videos.likes/videos.views as "Ratio of Likes to Views" 
from videos
join channels on channels.channelid = videos.channelid
join creators on channels.userid = creators.userid
join users on users.userid = creators.userid 
where users.name like '%Marvel Entertainment%'
order by videos.title asc;

-- query 2 v2
select videos.title as "Video Name", channels.name as "Channel Name", videos.likes/videos.views as "Likes to Views Ratio"
from videos 
join channels on videos.channelid = channels.channelid 
where videos.channelid in (select channels.channelid 
from channels 
where channels.userid in 
(select creators.userid from creators where creators.userid in 
(select users.userid from users where users.name like '%Marvel Entertainment%')))
order by videos.title asc;

-- query 2 v3
select videos.title as "Video Title", channels.name as "Channel Name", videos.likes/videos.views as "Ratio of Likes to Views"
from videos
join channels on channels.channelid = videos.channelid
join creators on creators.userid = channels.userid 
where exists
(select * from users where users.userid = creators.userid and users.name like '%Marvel Entertainment%')
...
-- query 4
select 
informational_videos.keyword as 'Informational Videos Keyword', avg(comments.sentiment) as 'Average Sentiment Score' 
from informational_videos
join comments on comments.videoid =informational_videos.videoid
group by informational_videos.keyword
order by avg(comments.sentiment) desc;

-- query 5

select videos.title as "Video Title", MIN(users.age) as "Minimum User Age", MAX(users.age) as "Maximum User Age"
from videos, users, comments
where users.userid = comments.userid 
and videos.videoid = comments.videoid and videos.channelid 
in (select channelid from channels where channels.userid in (select userid from users where users.name like '%Taylor Swift%'))
...
select users.name as "User Name", channels.name as "Channel Name", creators.subscriber_count as "Total Subscribers of the User"
from users
join creators on creators.userid = users.userid 
join channels on channels.userid = creators.userid
where users.address like '%United States of America%'
and channels.channelid in 
(select channelid from 
(select channelid as "channelid", WEEK(uploadtime) as "weekupload", YEAR(uploadtime) as "yearupload" 
from videos 
WHERE uploadtime BETWEEN '2023-01-01' AND '2023-01-31' 
GROUP BY channelid, WEEK(uploadtime), YEAR(uploadtime) having COUNT(*) > 0) t1 group by channelid having count(*) >= 4);