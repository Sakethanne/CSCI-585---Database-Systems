...
/*
 * Queries required to run this query are the below create table and the insert queries.
 * Here are the insert queries that are required to show the given output. Here the sponsor with the sponsor id 2 have sponsored the most amount in youtube and has sponsored more than 1 videos.
 */
...
/*
 * Step 1: SELECT CLAUSE: the select clause will be used to select/project the sponsors name, sponsors phone number, total sponsored amount.
 * Step 2: GROUP BY CLAUSE: the total sponsored amount will be given by using the "sum" aggregate function on the field sponsors.amount field, 
 * 			when this data is grouped by the sponsors.name and sponsors.contact it gives us the details of the sponsor name, their contact and the total sum of the amount that they have sponsored.
 * Step 3: ORDER BY CLAUSE: by ordering the sum(sponsors.amount) in the descending order, we shall get the details of the sponsors who have sponsored the most amount of vedios on youtube.
 * Step 4: LIMIT CLAUSE: by limiting the results of the query to one, we shall get the details of the sponsor who has done the most sponsorships in youtube.
*/
