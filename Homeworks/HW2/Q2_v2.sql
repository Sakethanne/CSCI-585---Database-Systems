...
/*
 * Below are the queries that can be used to insert the data into the required tables.
 * For this question we need to have the following tables, i.e., Users, Creators, Channels and Videos
 */
...
/*
 * Step 1: SELECT CLAUSE: the select clause will be used to select/project the Videos title from the videos table, Channel name from the channels table, the raio of number of likes : number of views from the videos table.
 * Step 2: FROM CLAUSE: Here we are using the users channels and videos tables beause we are extracting data from them
 * Step 3: WHERE CLAUSE: the where clause shall be used filter the users table to get the user id of the user who contain "Marvel Entertainment" in the users name.
 * 						and also map the records from the videos table and the channels table using the channelid column as the bridge, and then map the channels table and users table using the user id column
 * Step 4: ORDER BY CLAUSE: We need to order the records in ascenting order of the video title, so we are using the order by on the videos.title column
 * Step 5: SUB QUERY: I am using the subqueries to return the respective results, First I am find the user id from the users table whose name contains 'Marvel Entertainment' in them
					then I am mapping that user id with the creators table to see if the user has channels or not, and then using the user id to find all the channels that he owns
                    then i am using that channel id to find all the videos that those channels have published, and for each of those videos, i am finding the ratio of the likes to views and the displaying the channel name.
*/
