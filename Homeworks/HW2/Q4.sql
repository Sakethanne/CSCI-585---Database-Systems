...
/*
 * The below are the queries that are required to create the tables required and insert the data into them
 */
create database DB;
...
/*
 * Below is the query to return the Keyword and its sentiment score
 * SELECT CLAUSE: We use the select clause to project the columns Keywords from the informational_videos table and thhe average snetiment score from the comments table.
 * JOIN CLAUSE: We join the informational_vedios table with the comments table to extract the comments of only the informational vedios using the vedio id column
 * GROUP BY CLAUSE: Since we need to find the average sentiment score of all the videos for a given keyword, we shall group all the similar records using that keyword
 * ORDER BY CLAUSE: since we nned to order the records in the descending order such that the ones with the highest average sentiment score are displayed on the top, we use the 'desc' in the ORDER BY
 */
