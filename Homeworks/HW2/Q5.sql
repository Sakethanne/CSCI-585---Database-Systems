...
/*
 * Below are the queries that are required to create the tables and insert the data into the tables for the queries to execute smoothly
 */
...
/*
 * In the below query, we are using the Select statement to project the Video Title, Maximum of the users age and the minimum of the users age
 * FROM CLAUSE: Here we are using three tables, videos, users, and comments
 * WHERE CLAUSE: We need to match the records, from the users table and teh comments table based on the userid column, 
 * 	from the videos table and the comments table using the videoid column, and now, we need to get the channel id of all the channels that are owned by the user 'Taylor Swift'
 * 	this is done by using sub queries: We first select the userid from the users taable where the user name is 'Taylor Swift' and then match the userid with the userid/owner id in the channels table to extract the channel id
 * 	this channel id is used to extract the videos from the videos table based channel id from the videos table.
 * GROUP BY: we are using the group by clause to join the records that have of the videoid that have the more number of comments
 * ORDER BY: Since we need to only get the records that have the maximum number of comments, we are using the order by clause with the descending order
 * LIMIT: Since we only need one record, we are using the limit statement
 */
