...
/*
 * SUBQUERY1: The sub query 1 finds the channel from the videos table that has been consistently publishing videos evey week in the month of 'january' using the group by command on the 'week' so that all the videos in a week are combined into 1.
 * SUBQUERY2: the sub query 2 is used to find the channel id by grouping the week from the sub query 1 and and such that the count of records is more than or equal to 4 which means that there is a video in every week of the month january.
 * SELECT CLAUSE: The select clause is used to project the user name, channel name and the total subscribers of the user from the users table
 * JOIN CLAUSE: Then the results are joined with the creators and the channels table to find the userid of the channel creator and then map it with the users table to get the name of the user using the foreign key column userid
 * WHERE CLAUSE: The where clause is used to fiilter down the results to the user that resides in the United States of America.
 */
